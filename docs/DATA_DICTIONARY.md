...
### sentences
RAG source — Day 20. Currently 0 rows. Populated Day 15.
...
| sentence_type | TEXT | Classified Day 15 |
| sentiment | TEXT | Classified Day 15 |

---

### persona_calls
Voting ledger and accuracy tracker. One row per agent per ticker per run. Meta-Agent excluded.
...
| ticker | TEXT NOT NULL | e.g. NVDA |
| direction | TEXT | ACCUMULATE / HOLD / REDUCE / EXIT |
| confidence_score | INTEGER | 1-5 self-estimated conviction |
| regime_tag | TEXT | low_vix / normal / high_vix / crisis at call time |
| vix_level | REAL | VIX price at call time |
| rationale_summary | TEXT | First 200 chars of key reasoning field |
| outcome | TEXT | correct / incorrect / partial / void — T-3 stub |
| resolved_by_run_id | TEXT | Run that scored outcome |
| outcome_5d | TEXT | Scored at +5 trading days — added Day 13 |
| outcome_20d | TEXT | Scored at +20 trading days — added Day 13 |
| price_at_signal | REAL | Price at moment of call — immutable baseline for scoring |

---

### balance_ledger
Manual cost tracking. Anthropic Usage API not available on individual plans.

| Column | Type | Description |
|---|---|---|
| id | INTEGER PK | Autoincrement |
| timestamp | TEXT NOT NULL | ISO datetime |
| entry_type | TEXT NOT NULL | topup / reconcile / adjustment |
| amount_usd | REAL NOT NULL | Positive for topup, negative for adjustment |
| balance_after_usd | REAL | Estimated remaining after this entry |
| notes | TEXT | e.g. "June topup" |
...
| id | INTEGER PK | Autoincrement |
| timestamp | TEXT NOT NULL | ISO datetime |
| ticker | TEXT NOT NULL | Ticker the draft applies to |
| section | TEXT NOT NULL | Thesis section being proposed |
| current_text | TEXT | Current text in TICKER_THESIS |
| proposed_text | TEXT NOT NULL | AI-proposed replacement |
| confidence | INTEGER | 1-5 AI confidence |
| trigger_signal | TEXT | Signal that prompted draft |
...
| call_llm(prompt, system, model, max_tokens, temperature, fallback_model, client, call_type) | (str, dict) | Universal Claude API wrapper. Handles fixtures, fallback, truncation retry, capture |
...
| save_price_fixtures(price_data) | None | Updates normal_day.json prices block |
| send_email_alert(subject, body) | bool | Gmail SMTP alert. Returns True on success |
